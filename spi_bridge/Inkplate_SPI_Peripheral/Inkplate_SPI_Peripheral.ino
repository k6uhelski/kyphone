...
void render_lock(char* data) {
    ...
    // "OS 0.1" — bottom left, textSize 1
    display.setTextSize(1);
    display.setCursor(10, 584);
    display.print("OS 0.1");

    // ASCII cat — bottom right, drawn as a fixed bitmap (see cat_bitmap above)
    display.drawBitmap(600 - CAT_BITMAP_W - 6, 600 - CAT_BITMAP_H - 6, cat_bitmap, CAT_BITMAP_W, CAT_BITMAP_H, BLACK);

    // Clock — textSize 8 (48px/char, 64px tall), centered at y=190
    display.setTextSize(8);
    int clock_w = strlen(time_str) * 48;
    display.setCursor((600 - clock_w) / 2, 190);
    display.print(time_str);

    // Date — textSize 2 (12px/char), centered at y=270
    display.setTextSize(2);
    int date_w = strlen(date_str) * 12;
    display.setCursor((600 - date_w) / 2, 270);
    display.print(date_str);

    // Quote — word-wrapped, centered, starting at y=390
    display.setTextSize(2);
    int quote_end_y = render_centered_wrapped(quote_buf, 390, 20, 2);
...
void render_home2(char* data) {
    ...
    const int num_w    = 36; // 2 chars at textSize 3 = 36px
    const int gap      = 20;
    ...
    for (int i = 0; i < 4; i++) {
        ...
        // App label — textSize 6, bold (double-print) (was 4)
...
void render_thread2(char* data) {
    ...
    if (back_sel) {
        display.fillRect(0, 0, 60, 46, BLACK);
        display.setTextColor(WHITE);
    } else {
        display.setTextColor(BLACK);
    }
    ...
    if (info_sel) {
        display.fillRect(540, 0, 60, 46, BLACK);
        display.setTextColor(WHITE);
    } else {
        display.setTextColor(BLACK);
    }
...
void render_compose(char* data) {
    ...
    if (p1) {
        ...
        if (p2) {
            ...
            char* p3 = strchr(p2 + 1, '|');
            if (p3) {
                snprintf(hdr_buf, sizeof(hdr_buf), "%.*s", (int)(p3 - p2 - 2), p2 + 2);
            }
        } else {
    ...
    // X — top right, exits (same as Esc)
    if (x_sel) {
        display.fillRect(540, 0, 60, 44, BLACK);
        display.setTextColor(WHITE);
    } else {
        display.setTextColor(BLACK);
    }
    display.setCursor(600 - 16 - 18, 8);
    display.print("X");
    display.setTextColor(BLACK);
    ...
    display.setTextSize(2);
    display.setTextColor(BLACK);
    display.setCursor(600 - 24 - 4 * 12, 570);
    display.print("SEND");
}
